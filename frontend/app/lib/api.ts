...
// specs/004-two-stage-analysis: analyze -> resolve-gaps (repeatable) -> run.

export async function analyzeProject(file: File, companyName?: string, companyContext?: string) {
  const formData = new FormData();
  formData.append('file', file);
  if (companyName) formData.append('company_name', companyName);
  if (companyContext) formData.append('company_context', companyContext);

  const response = await fetch(`${API_URL}/api/v1/compliance-check/analyze`, {
    method: 'POST',
    body: formData,
  });
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => null);
    throw new Error(detailToMessage(error?.detail, 'Impossible d’analyser le projet.'));
  }

  return response.json();
}

export type GapAnswer = { gap_id: string; text: string };

export async function resolveGaps(sessionId: string, answers: GapAnswer[], file?: File) {
  const formData = new FormData();
  if (answers.length > 0) formData.append('answers', JSON.stringify(answers));
  if (file) formData.append('file', file);

  const response = await fetch(`${API_URL}/api/v1/compliance-check/${sessionId}/resolve-gaps`, {
    method: 'POST',
    body: formData,
  });
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => null);
    throw new Error(detailToMessage(error?.detail, 'Impossible d’envoyer ces informations.'));
  }

  return response.json();
}

export async function runAnalyzedCheck(sessionId: string) {
  const response = await fetch(`${API_URL}/api/v1/compliance-check/${sessionId}/run`, {
    method: 'POST',
  });

  if (!response.ok) {
    const error = await response.json().catch(() => null);
    throw new Error(detailToMessage(error?.detail, 'Impossible de lancer le remplissage du formulaire.'));
  }
